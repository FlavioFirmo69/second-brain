...
async def answer(session: Session, raw_text: str, history: list[dict[str, str]] | None = None) -> dict:
    ...
    event = re.fullmatch(r"(?:stasera|oggi)(?:\s+alle)?\s+(\d{1,2})(?::(\d{2}))?\s+(.+)", command)
    if event:
    ...
    event = re.fullmatch(r"domani(?:\s+alle)?\s+(\d{1,2})(?::(\d{2}))?\s+(.+)", command)
    if event:
        hour, minute, title = event.groups()
        item = repo.create_event({
...
{{"action":"create_event","title":"...","event_date":"YYYY-MM-DD","start_time":"HH:MM:SS o null","location":null,"event_type":"personal"}}
{{"action":"create_events","events":[{{"title":"...","event_date":"YYYY-MM-DD","start_time":"HH:MM:SS o null","end_time":"HH:MM:SS o null","location":null,"event_type":"personal"}}]}}
{{"action":"create_task","title":"...","due_date":"YYYY-MM-DD o null","due_time":"HH:MM:SS o null","priority":3,"project_code":"codice progetto o null"}}
{{"action":"respond","text":"..."}}
...
Non creare vendite o movimenti finanziari. Se la richiesta è ambigua usa respond e chiedi conferma."""},
        ...
        response = await complete(messages, response_format={
            ...
            "json_schema": {
                ...
                "schema": {
                    "type": "object",
                    "properties": {
                        "action": {"type": "string", "enum": ["create_event", "create_events", "create_task", "respond", "draft_article"]},
                        ...
                        "events": {
                            ...
                            "items": {
                                ...
                                "required": ["title", "event_date", "start_time", "end_time", "location", "event_type"],
                    ...
                    "required": ["action", "text", "title", "event_date", "start_time", "location", "event_type", "events", "due_date", "due_time", "priority", "project_code", "subtitle", "content_markdown", "author_code"],
                    "additionalProperties": False,
                },
            },
        })
        ...
        if action == "create_task":
            ...
            item = repo.create_task({
                ...
                "project_id": repo.project_id_by_code(payload.get("project_code")), "case_id": None, "author_profile_id": None, "book_id": None,
            })
