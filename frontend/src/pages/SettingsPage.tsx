...
import type { Profile, SystemInfo } from '../types'
...
export function SettingsPage() {
  ...
  useEffect(() => { void load(); void api<{content_markdown:string}>('/manuals/deterministic-rules').then(data=>setRules(data.content_markdown)); void api<SystemInfo>('/system/info').then(setSystemInfo) }, [])
  ...
  </Panel><Panel title="Regole deterministiche"><p className="read-only-note">Promemoria in sola lettura · applicate direttamente dal backend</p>{rules ? <MarkdownView value={rules}/> : <p>Caricamento…</p>}</Panel><Panel title="Informazioni applicazione">{systemInfo?<dl className="system-info"><div><dt>Versione</dt><dd>v{systemInfo.version}</dd></div><div><dt>Ambiente</dt><dd>{systemInfo.environment}</dd></div><div><dt>LLM</dt><dd>{systemInfo.llm_enabled?(systemInfo.llm_model||'Attivo'):'Disattivato'}</dd></div></dl>:<p>Caricamento…</p>}</Panel></div>
}
